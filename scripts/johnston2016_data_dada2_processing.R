...
####CHANGE THESE####
PATH="/Users/mann/github/triatomes/johnston2016_topsoil/" #CHANGE ME to your working directory
RAW="/Users/mann/github/triatomes/johnston2016_topsoil/raw" #CHANGE ME to where your raw fastq files are
...
RVPAT="_2.fastq" #CHANGE ME to match the pattern in your reverse reads
...
####trim & filter####
# quality scores dip at 5' and 3' end of forward and reverse reads, these are 2x150 reads, so trim max 10bp off each end -- see how this effects merging rates later
out <- filterAndTrim(cutFs, filtFs, cutRs, filtRs, trimLeft=10, trimRight=10, minLen = c(100,100),
...
seqtab.nochim <- removeBimeraDenovo(seqtab.nosingletons, method="pooled", multithread=TRUE, verbose=TRUE) #this step can take a few minutes to a few hours, depending on the size of your dataset
dim(seqtab.nochim)
sum(seqtab.nochim)/sum(seqtab.nosingletons) #proportion of nonchimeras #it should be relatively high after filtering out your singletons/low-count ASVs, even if you lose a lot of ASVs, the number of reads lost should be quite low

####track read retention through steps####
getN <- function(x) sum(getUniques(x))
track <- cbind(out[samples_to_keep,], sapply(dadaFs[samples_to_keep], getN), sapply(dadaRs[samples_to_keep], getN), sapply(mergers, getN), rowSums(seqtab.nosingletons), rowSums(seqtab.nochim))
...
system2("awk '/^>/{print \">ASV\" ++i; next}{print}' < rep_set.fa > rep_set_fix.fa")
...
my_otu_table <- t(as.data.frame(seqtab.nochim)) #transposed (OTUs are rows) data frame. unclassing the otu_table() output avoids type/class errors later on
...
colnames(seqtab.nochim) <- ASV.num #rename your ASVs in the taxonomy table and sequence table objects
...
taxa <- read.table("assigntax/rep_set_tax_assignments.txt", header=F, sep="\t", row.names=1)
